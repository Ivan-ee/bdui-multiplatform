...
Текущее состояние: **вертикальный срез** — экран каталога собирается на бэкенде из живых
данных Postgres и рендерится одним и тем же DivKit-JSON на Android и в веб-кабинете
(проверено), iOS собирается и рендерит тот же JSON через SDK (сетевой слой пока не shared).
...
- `GET /api/v1/objects` — список свободных Объектов (JSON по модели из тикета 07)
- `GET /api/v1/layout/catalog` — карта каталога: конверт DivKit `{templates, card}`,
  собирается на сервере из живых данных; один и тот же JSON рендерят все три клиента
